import { Request, Response } from 'express';
export declare function createProject(req: Request, res: Response): Promise<void>;
export declare function createProjectChunk(req: Request, res: Response): Promise<void>;
export declare function getAllProjects(req: Request, res: Response): Promise<void>;
export declare function getProject(req: Request, res: Response): Promise<void>;
export declare function updateProject(req: Request, res: Response): Promise<void>;
export declare function updateProjectChunk(req: Request, res: Response): Promise<void>;
//# sourceMappingURL=projectController.d.ts.map