...
export default function ApDashboardPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await finApi.fetchApDashboard();
      setData(res);
    } catch (err: any) {
      showToast(err.message || 'Failed to load AP dashboard', 'error');
    } finally {
      setLoading(false);
    }
  }, [showToast]);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4 bg-gradient-to-r from-rose-950 via-rose-900 to-slate-900 p-6 rounded-2xl text-white shadow-xl">
        ...
        <div>
          <h1 className="text-xl font-bold font-display tracking-tight text-white">Accounts Payable</h1>
          <p className="text-xs text-rose-200/80 mt-0.5">What the company currently owes to vendors.</p>
        </div>
      </div>
      ...
      {loading || !data ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <StatCard label="Total Outstanding" value={fmtMoney(data.totalOutstanding)} icon={Wallet} tone="bg-rose-50 text-rose-600" />
