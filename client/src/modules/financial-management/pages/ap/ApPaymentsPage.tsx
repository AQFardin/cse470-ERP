...
import type { VendorPayment, PaymentRecordStatus } from '../../types';
...
export default function ApPaymentsPage() {
  ...
  const canReverse = hasPermission('ap', 'reverse_payment');
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const res = await finApi.fetchVendorPayments({ status: statusFilter, pageSize: 50 });
      setPayments(res.payments);
    } catch (err: any) {
      showToast(err.message || 'Failed to load vendor payments', 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleReverse = async (id: string, paymentNumber: string) => {
    ...
    try { await finApi.reverseVendorPayment(id); showToast(`${paymentNumber} reversed`, 'success'); load(); } catch (err: any) { showToast(err.message, 'error'); }
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4 bg-gradient-to-r from-rose-950 via-rose-900 to-slate-900 p-6 rounded-2xl text-white shadow-xl">
        <div className="w-12 h-12 rounded-xl bg-rose-500/20 border border-rose-400/30 flex items-center justify-center text-rose-300 shrink-0"><Wallet className="w-6 h-6" /></div>
        <div><h1 className="text-xl font-bold font-display tracking-tight text-white">Vendor Payments</h1><p className="text-xs text-rose-200/80 mt-0.5">Every payment posted against a vendor bill.</p></div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {payments.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50/70">
                  ...
                  <td className="px-4 py-3 font-mono text-gray-600">{p.vendorBill?.billNumber}</td>
                  <td className="px-4 py-3">{p.vendorBill?.vendor.name}</td>
